...
import { getAllTopics, getQuestionsByTopic } from '../utils/javascriptPracticeQuestions';
...
const JavascriptQuestions = () => {
    ...
    const handleTaskClick = (id) => {
        navigate(`/javascript-practice/${id}`)
    }
    ...
    return (
        <Wrapper>
            ...
            <Container>
                ...
                <Description>Sharpen your coding skills with these JavaScript practice problems!</Description>
                <TaskGridMain>
                    {getAllTopics()?.map((topic) => (
                        <>
                            <DifficultyTitle>{topic}</DifficultyTitle>
                            <TaskGrid>
                                {getQuestionsByTopic(topic)?.map((question) => (
                                    <TaskCard key={question?.id} onClick={() => handleTaskClick(question?.id)}>
                                        <TaskHeader>
                                            <TaskTitle>{question?.question?.substring(8)}</TaskTitle>
                                            <span onClick={(e) => {
                                                e.stopPropagation();  // Prevents the TaskCard onClick from being triggered
                                                handleTaskStatusClick(question?.id)
                                            }}>{statusIcons[taskStatuses[question.id]]}</span>
                                        </TaskHeader>
                                    </TaskCard>
                                ))}
                            </TaskGrid></>
